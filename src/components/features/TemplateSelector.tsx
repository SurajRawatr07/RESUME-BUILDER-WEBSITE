...
import { X, Check, CheckCircle2, Filter, UserCheck, Sparkles } from 'lucide-react';
...
export default function TemplateSelector({ onClose }: TemplateSelectorProps) {
  ...
  const [useLiveUserData, setUseLiveUserData] = useState(true);
  ...
  const handleSelect = (templateId: TemplateType) => {
    ...
    userPreferencesStorage.setLastSelectedTemplate(templateId);
    onClose();
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Categories Bar & Live Data Toggle */}
        <div
          className={`px-6 py-2.5 border-b flex items-center justify-between gap-3 overflow-x-auto shrink-0 ${
        ...
        >
          ...
          <button
            onClick={() => setUseLiveUserData(!useLiveUserData)}
            className={`px-2.5 py-1 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors shrink-0 border ${
              useLiveUserData
          ...
          >
            ...
            <span>{useLiveUserData ? 'Previewing My Data' : 'Previewing Sample'}</span>
          </button>
        </div>
              ...
              >
                ...
                <div className="relative p-3 bg-gray-100 dark:bg-gray-950 flex justify-center items-center border-b border-gray-200 dark:border-gray-800">
                  <div className="w-full max-w-[190px]">
                    <TemplateMiniPreview template={template} liveData={useLiveUserData ? resumeData : undefined} />
                  </div>
